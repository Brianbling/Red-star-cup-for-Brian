...
- 缺点级处理：
  ...
  - **坐标归一化**：每只手独立，`(点 - 手腕) / ‖中指根 - 手腕‖₂`（`src/preprocess_keypoints.py` 的 `normalize_hand()`）。**兜底下限 scale<0.02 时用 0.1**（2026-08-02 审计修复：原 1e-6 兜底会把 1e-6~0.02 的极小 scale 放大 12~650 倍），消除远近拍摄影响。E: 全部 1,247,783 帧已验证 wrist=[0,0]、腕→中指根距离=1.0
