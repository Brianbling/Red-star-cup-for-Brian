...
- **适用场景**：v1 即可用，训练前加即可
- **状态**：**已实现**（src/model.py 用两个 stride=2 Conv1d 总降采样 /4，无显式 lgt 追踪）
...
- **状态**：**已实验并否决**（2026-07-31 beam=3 与贪心 WER 完全一致，0pp。模型 P(blank)≈0.77，概率分布过于尖锐，无备选路径可探索。只改解码不改模型架构无价值）
...
- **适用场景**：v1 可直接采用——替换当前 `presence < 0.6 → 置零` 为 `→ NaN`
- **状态**：**未采纳**（当前数据用 0 作为缺失标志 + activity_detect 切手部丢失帧已覆盖该语义；改动收益低，风险高，未实验）
...
- **状态**：待讨论（当前 (T,84) 扁平格式已稳定，改动影响全部 .npy 与模型输入，未实验）
...
- **状态**：**部分完成**（activity_detect 已做手部丢失段分析，发现 99.7% 丢失帧为零帧、min_active_frames=5 无误杀；未做 signer/词类级热力图）
...
- **状态**：待讨论（当前 decode.py 已做连续重复合并 + blank 过滤；CSL 标注无 `__LEFTHAND__` 类噪声标记，收益待验证）
...
**⚠ 2026-07-31 实验裁决**：#10 的 beam search（beam=3）已实测与贪心 WER 完全一致（0pp）。根因是模型 P(blank)≈0.77，概率分布过于尖锐，无备选路径可探索。因此升级到 v2 的 beam search 在当前模型下无价值——除非模型架构或数据有根本变化（如数据扩展后模型输出更"犹豫"）。v1 的贪心解码维持不动。
...
- 状态：「待讨论」「v1 采纳」「v2 保留」「不采纳」「参考」「已实现」「已实验并否决」
- **实验裁决已更新**（2026-07-31/08-01）：#3 时序增强、#10 CTC Prefix beam search 已实验并否决；#1 TemporalConv 已实现（两个 stride=2 级联）；#15 NaN、#16 双手张量未采纳/待讨论。详见各条状态
...
### LL-5: YOLO "pretrained=True" 不加载预训练权重（ultralytics 8.4.105）
- **日期**：2026-08-01
- **问题**：`YOLO("yolov8s.yaml").train(pretrained=True)` 训练 L1290 首轮 7 epoch，mAP 明显差于预期
- **根因**：yaml 构建的模型没有 ckpt，bool 型 `pretrained=True` 不触发 `load_checkpoint`（训练 log 里没有 "Transferred" 行）——等于随机初始化。必须传 `.pt` 文件：`YOLO("yolov8s.pt")` 才会加载 COCO 权重。另：GitHub 下载 SSL 证书验证失败（被墙/证书），需 `ssl.CERT_NONE` + urllib 手动下载 yolov8s.pt（22.6MB）
